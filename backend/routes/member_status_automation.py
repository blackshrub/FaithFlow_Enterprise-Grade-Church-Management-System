...
async def update_status(
    ...
    if current_user.get('role') != 'super_admin' and current_user.get('church_id') != status_obj.get('church_id'):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
...
async def create_rule(
    ...
    if current_user.get('role') != 'super_admin' and current_user.get('church_id') != rule_data.church_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
...
async def update_rule(
    ...
    if current_user.get('role') != 'super_admin' and current_user.get('church_id') != rule.get('church_id'):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
