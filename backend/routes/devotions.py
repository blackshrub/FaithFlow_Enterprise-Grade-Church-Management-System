...
async def get_devotion(
    ...
    if current_user.get('role') != 'super_admin' and current_user.get('church_id') != devotion.get('church_id'):
...
async def create_devotion(
    ...
    if current_user.get('role') != 'super_admin' and current_user.get('church_id') != devotion_data.church_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
...
async def duplicate_devotion(
    ...
    if current_user.get('role') != 'super_admin' and current_user.get('church_id') != original.get('church_id'):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
