...
import { Baby, Phone, Calendar, ArrowLeft, MoreHorizontal, Check, MessageCircle, CalendarDays, Users, Trash2 } from 'lucide-react';
...
import { useRequestsListByType, useMarkContacted, useMarkScheduled, useMarkCompleted, useDeleteRequest } from '../../hooks/useRequestForms';
...
const ChildDedicationList = () => {
  ...
  const { data: response, isLoading, error } = useRequestsListByType('child_dedication', {
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold">
              {isIndonesian ? 'Penyerahan Anak' : 'Child Dedication'}
            </h1>
          </div>
          <p className="text-gray-500 mt-1">
            {t('requestForms.childDedicationSubtitle', 'Child dedication requests and scheduling')}
          </p>
        </div>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {requests.map((request) => {
                const childData = request.child_dedication_data?.child;
                return (
                  <div
                    key={request.id}
                    className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors"
                    onClick={() => navigate(`/request-forms/child-dedication/${request.id}`)}
                  >
                    <div className="flex items-center gap-3">
                      <MemberAvatar
                        name={childData?.name || request.full_name}
                        photo={childData?.photo_url || request.member_photo}
                        size="md"
                      />
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-medium">{childData?.name || 'Child'}</span>
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-500 mt-1">
                          ...
                          {childData?.birth_date && (
                            <Badge variant="outline" className="text-xs">
                              Born: {format(parseISO(childData.birth_date), 'MMM dd, yyyy')}
                            </Badge>
                          )}
                        </div>
                      </div>
