...
import { Droplets, Phone, Calendar, ArrowLeft, MoreHorizontal, Check, MessageCircle, CalendarDays, Trash2 } from 'lucide-react';
...
const BaptismList = () => {
  ...
  const { data: response, isLoading, error } = useRequestsListByType('baptism', {
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold">
              {isIndonesian ? 'Baptis' : 'Baptism'}
            </h1>
          </div>
          <p className="text-gray-500 mt-1">
            {t('requestForms.baptismSubtitle', 'Baptism requests and scheduling')}
          </p>
        </div>
      </div>
            ...
            <div className="space-y-3">
              {requests.map((request) => (
                <div
                  ...
                  onClick={() => navigate(`/request-forms/baptism/${request.id}`)}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-sm text-gray-500 mt-1">
                        ...
                        {request.baptism_data?.preferred_date && (
                          <Badge variant="outline" className="text-xs">
                            <CalendarDays className="h-3 w-3 mr-1" />
                            {format(parseISO(request.baptism_data.preferred_date), 'MMM dd')}
                          </Badge>
                        )}
                      </div>
                    </div>
