...
import { Gem, Phone, Calendar, ArrowLeft, MoreHorizontal, Check, MessageCircle, CalendarDays, Heart, Trash2 } from 'lucide-react';
...
const HolyMatrimonyList = () => {
  ...
  const { data: response, isLoading, error } = useRequestsListByType('holy_matrimony', {
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold">
              {isIndonesian ? 'Pernikahan Kudus' : 'Holy Matrimony'}
            </h1>
          </div>
          <p className="text-gray-500 mt-1">
            {t('requestForms.holyMatrimonySubtitle', 'Wedding requests and scheduling')}
          </p>
        </div>
      </div>
              ...
              {requests.map((request) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => navigate(`/request-forms/holy-matrimony/${request.id}`)}
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex -space-x-2">
                        <MemberAvatar
                          name={personA.name || 'Person A'}
                          photo={personA.photo_url}
                          size="md"
                          className="ring-2 ring-white"
                        />
                        <MemberAvatar
                          name={personB.name || 'Person B'}
                          photo={personB.photo_url}
                          size="md"
                          className="ring-2 ring-white"
                        />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-medium">
                            ...
                            <Heart className="inline h-3 w-3 text-rose-500 mx-1" />{' '}
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-500 mt-1">
                          ...
                          {matrimonyData.planned_wedding_date && (
                            <Badge variant="outline" className="text-xs">
                              ...
                              {format(parseISO(matrimonyData.planned_wedding_date), 'MMM dd, yyyy')}
                            </Badge>
                          )}
...
export default HolyMatrimonyList;
